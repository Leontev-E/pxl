(function () {
    ...
    function hostnameSafe() {
        try {
            return (window.location.hostname || "host").replace(/\s+/g, "");
        } catch (e) {
            return "host";
        }
    }
    ...
    function replacePlaceholderInputs(subid) {
        ...
        try {
            inputs = document.querySelectorAll(
                'form input[value="' +
                    PLACEHOLDER_SUBID +
                    '"], form input[data-boostclicks-subid="1"]',
            );
        } catch (e) {
            inputs = [];
        }
    ...
    function writeCapRaw(raw, ttlSec) {
        try {
            localStorage.setItem(SUBMIT_CAP_KEY, raw);
        } catch (e) {}
    ...
    function getCapState() {
        ...
        try {
            ...
            var count =
                obj && typeof obj.count !== "undefined"
    ...
    function isLikelyLeadForm(form) {
        ...
        try {
            ...
            if (
                form.querySelector(
                    'input[type="tel"], input[name="phone"], input[name*="phone" i], input[name*="tel" i], input[name*="тел" i]',
                )
            )
                return true;
            if (
                form.querySelector(
                    'input[value="' +
                        PLACEHOLDER_SUBID +
                        '"], input[name*="subid" i], input[data-boostclicks-subid="1"]',
                )
            )
                return true;
        } catch (e) {}
    ...
    function enforceDailySubmitCap(form) {
        ...
        if (st.count >= SUBMIT_CAP_MAX) {
            try {
                alert(
                    "Лимит отправок для этого устройства: " +
                        SUBMIT_CAP_MAX +
                        " в сутки. Попробуйте завтра.",
                );
            } catch (e) {}
            return false;
        }
    ...
    function pingKeitaroNamePhone(keitaroSubid, nameClean, phoneClean) {
        ...
        try {
            var lastTs =
                parseInt(
                    sessionStorage.getItem(KEITARO_PING_TS_KEY) || "0",
                    10,
                ) || 0;
            var nowTs = Date.now();
        ...
        try {
            ...
            var url =
                ...
                "?_update_tokens=1" +
                "&sub_id=" +
                encodeURIComponent(String(keitaroSubid)) +
                "&sub_id_22=" +
                encodeURIComponent(String(nameClean)) +
                "&sub_id_23=" +
                encodeURIComponent(String(phoneClean));
    ...
    function captureLeadFieldsFromForm(form, subid) {
        ...
        try {
            if (window.FormData) {
                ...
                nameVal = pickFromFormData(
                    fd,
                    /^(name|fullname|fio|first_name|last_name|имя)$/i,
                );
                phoneVal = pickFromFormData(
                    fd,
                    /^(phone|tel|telephone|mobile|phone_number|телефон)$/i,
                );
            }
        } catch (e) {}
        ...
        if (!nameVal) {
            try {
                var elName = form.querySelector(
                    'input[name="name"], input[name="fullname"], input[name="fio"], input[name*="name" i], input[name*="fio" i], input[name*="имя" i]',
                );
        ...
        if (!phoneVal) {
            try {
                var elPhone = form.querySelector(
                    'input[type="tel"], input[name="phone"], input[name*="phone" i], input[name*="tel" i], input[name*="тел" i]',
                );
    ...
    window.__boostclicksDisableTimeOnSite =
        window.__boostclicksDisableTimeOnSite || false;
    ...
    function attachSubmitHandler() {
        ...
        document.addEventListener(
            ...
            function (ev) {
                ...
                var sid = null;
                try {
                    sid = ensureSubidApplied();
                } catch (e) {
                    sid = null;
                }
                try {
                    var form = ev && ev.target ? ev.target : null;
                    captureLeadFieldsFromForm(
                        form,
                        sid || getSession("external_id"),
                    );
                } catch (e) {}
            },
            true,
...
(function () {
    ...
    function getDomonetkaFromGlobal() {
        try {
            if (typeof domonetka !== "undefined") {
                ...
                if (v && v !== "{domonetka}" && /^https?:\/\//i.test(v))
                    return v;
            }
        } catch (e) {}
        return "";
    ...
    function setDomonetkaSourceStorage(src) {
        ...
        if (!src) return;
        try {
            sessionStorage.setItem("domonetkaSourceBC", src);
        } catch (e) {}
        try {
            document.cookie =
                "domonetkaSourceBC=" +
    ...
    function setDomonetkaStorage(url, source) {
        ...
        if (!url) return;
        try {
            sessionStorage.setItem("domonetkaBC", url);
        } catch (e) {}
        try {
            document.cookie =
                "domonetkaBC=" +
    ...
    function getLandingParams() {
        var raw = "";
        try {
            raw = String(sessionStorage.getItem(LANDING_QS_KEY) || "");
        } catch (e) {
            raw = "";
        }
        if (!raw) {
            ...
            try {
                if (raw) sessionStorage.setItem(LANDING_QS_KEY, raw);
            } catch (e) {}
        }
    ...
    function showDomonetkaFrame(finalUrl) {
        ...
        try {
            var wrap = document.getElementById(
                "boostclicks-domonetka-frame-wrap",
            );
            if (!wrap) {
    ...
    function initDomonetka(domUrl, mode) {
        ...
        try {
            sessionStorage.setItem("dom", domUrl);
        } catch (e) {}
        ...
        try {
            window.onpopstate = function (event) {
                ...
                let finalUrl = domUrl;
                try {
                    finalUrl = buildDomonetkaUrlWithMappedParams(domUrl);
                } catch (e) {}
    ...
    function fetchDomonetkaForDomain(domain) {
        ...
        var url =
            "https://analytics.boostclicks.ru/api/domonetka.php?domain=" +
    ...
    if (domonetkaUrlNow) {
        if (domFromGlobal) {
            ...
            initDomonetka(
                domonetkaUrlNow,
                forceFrameBecauseGclid ? "frame" : "redirect",
            );
        } else {
            ...
            initDomonetka(
                domonetkaUrlNow,
                forceFrameBecauseGclid ? "frame" : baseMode,
            );
        }
    } else {
    ...
    try {
        if (!sessionStorage.getItem("external_id") && subid)
            setSessionItem("external_id", subid);
        if (!sessionStorage.getItem("event_id") && subid)
    ...
    (function initTikTokPageView() {
        const pixelId = (
            urlParams.get("pixel") ||
            sessionStorage.getItem("tt_pixel") ||
            ""
        ).trim();
        if (!pixelId) return;
...
// =========================================================================
// Forward _fbp / _fbc cookies to the CAPI bridge (collect.php) keyed by clickid.
// The Pixel script writes _fbp on init and _fbc when ?fbclid= is present in the
// URL. We push them server-side so the s2s postback fired by Keitaro on the
// thank-you page can include the same identifiers — this is the single biggest
// EMQ lever for server events.
// =========================================================================
(function () {
    var COLLECT_URL = "https://klm-team.pw/lander/capi_v2/collect.php";
    ...
    function getEstablishedClickid() {
        var c = getCookie("_subid");
        if (c && c !== "{subid}") return c;
        try {
    ...
    function buildFbc(fbclid, ts) {
        if (!fbclid) return "";
        if (/^fb\.\d+\.\d+\./.test(fbclid)) return fbclid;
    ...
    function postCollect() {
        var clickid = getEstablishedClickid();
        if (!clickid) return;
        ...
        if (!fbc) {
            try {
                var fbclid = new URLSearchParams(window.location.search).get(
                    "fbclid",
                );
    ...
    function schedule() {
        // First shot ~1.5s after load: gives fbevents.js time to write _fbp.
        setTimeout(postCollect, 1500);
        // Second shot at 4s in case the network was slow.
        setTimeout(postCollect, 4000);
    }
    ...
    // Also push on form submit so the freshest cookies travel with the lead.
    document.addEventListener(
        "submit",
        function () {
            setTimeout(postCollect, 50);
        },
        true,
    );

    // Last chance before the user navigates away.
    document.addEventListener("visibilitychange", function () {
        if (document.visibilityState === "hidden") postCollect();
    });
})();
