		...
		if (typeof domonetka !== 'undefined' && domonetka && domonetka.trim() !== '' && domonetka !== '{domonetka}') {
			document.addEventListener('DOMContentLoaded', () => {
					...
					function redirectToDomonetka() {
							window.onbeforeunload = null; // Отключаем перед редиректом
							const currentUrlParams = new URLSearchParams(window.location.search);
							const newUrlParams = new URLSearchParams();
					...
					// Отключаем предупреждение при отправке форм и кликах по ссылкам
					document.querySelectorAll('form').forEach(form => {
							form.addEventListener('submit', () => {
									window.onbeforeunload = null; // Отключаем при отправке формы
							});
					});

					document.querySelectorAll('a').forEach(link => {
							link.addEventListener('click', () => {
									window.onbeforeunload = null; // Отключаем при клике по ссылке
							});
					});

					// Установка события onbeforeunload
					window.onbeforeunload = function(e) {
							hasTriedToLeave = true;
							const confirmationMessage = "Вы действительно хотите покинуть сайт?";
							e.returnValue = confirmationMessage; // Устанавливаем сообщение для браузера
							setTimeout(function() {
							...
							return confirmationMessage; // Возвращаем строку
					};

					// Возвращаем visibilitychange
